...
public:
    ...
    //一下的函数，对于当前算法中，都是没用的，因为meshTriangle会马上调用getIntersection, 返回的intersection肯定是triangle类
...
inline Intersection Triangle::getIntersection(Ray ray)
{
    ...
    /*Intersection() //把构造函数放到这，提醒一下自己这个对象需要什么属性
    {
        happened = false;
        coords = Vector3f();
        normal = Vector3f();
    ...
    inter.normal = normal;
    inter.distance = dotProduct(inter.coords - ray.origin, inter.coords - ray.origin);
    inter.obj = this; 
    inter.m = m;
    return inter;
}
